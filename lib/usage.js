...
function combine() {
  ...
  if (oc) mergeUsage(lifetime, dayMap, byModel, byProject, oc);
  const codex = getCodexUsage();
  if (codex) mergeUsage(lifetime, dayMap, byModel, byProject, codex);
...
function mergeUsage(lifetime, dayMap, byModel, byProject, oc) {
